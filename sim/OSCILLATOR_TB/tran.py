...
def emit_latex_params(corners: list, _nominal_slope: float | None, outpath: Path) -> None:
    """Worst-case per-corner polyfit residuals across non-MC PVT corners.

    K_err,1 — best-linear (1st-order) polyfit T = a*f + b per corner.
    K_err,2 — quadratic (2nd-order) polyfit T = a*f^2 + b*f + c per corner.

    These match the table rows in §4 of the report, which the prose calls
    "1-pt" / "2-pt calibration" but really refer to per-corner polynomial
    fits of T vs f. \\providecommand means a hand value in params.tex
    always wins.
    """
    min_quadratic_pts = 3
    err_one_abs_max = 0.0
    ...
    for _label, proc, _volt_var, temps, freq, _idd in corners:
        if "mm" in proc:  # skip Monte Carlo runs
            continue
        if len(temps) < min_quadratic_pts:
            continue
        t_arr = np.array(temps, dtype=float)
        f_arr = np.array(freq)

        # 1st-order polyfit residual
        c1 = np.polyfit(f_arr, t_arr, 1)
        err_one = np.polyval(c1, f_arr) - t_arr
        err_one_abs_max = max(err_one_abs_max, float(np.max(np.abs(err_one))))

        # 2nd-order polyfit residual
        c2 = np.polyfit(f_arr, t_arr, 2)
        err_two = np.polyval(c2, f_arr) - t_arr
        err_two_abs_max = max(err_two_abs_max, float(np.max(np.abs(err_two))))
